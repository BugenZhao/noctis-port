#!/usr/bin/env python3
"""Rebuild the three light themes from the checked-in Noctis 10.40.0 sources.

Scope and semantic-selector scoring follow the pinned VS Code implementation
documented in docs/light-theme-port.md. No network or third-party modules needed.
"""
    ...
    for scopes in probes:
        ...
        if values:
            ...
            if "fontStyle" in values:
                ...
                # These three source themes have no standalone underline/strike
...
def generate():
    outputs = {}
    manifest = {"source_version": "10.40.0", "themes": {}}
    shared_rules = {}
    for variant in VARIANTS:
        source = json.loads((ROOT / f"reference/noctis-10.40.0/{variant}.json").read_text())
        path = ROOT / f"themes/{variant}-light.json"
        target = json.loads(path.read_text())
        target["$schema"] = "https://zed.dev/schema/themes/v0.2.0.json"
        theme = target["themes"][0]
        assert theme["appearance"] == "light"
        style = theme["style"]
        for key, original in UI_MAP.items():
        ...
        for key in list(style):
            if key.startswith("terminal.ansi.dim_") or key in ("terminal.dim_foreground", "terminal.bright_foreground"):
                del style[key]
        style["players"][0] = {
        ...
        style["syntax"] = syntax
        outputs[path] = target
        manifest["themes"][theme["name"]] = {"source": f"reference/noctis-10.40.0/{variant}.json",
...
def main():
    ...
    print(("Checked" if args.check else "Generated") + " three light themes and semantic mapping fragments.")
