# Light themes and semantic highlighting

Hibernus Light, Lilac Light and Lux Light are generated from the user's locally
installed **liviuschera.noctis 10.40.0**. The originals call these themes Noctis
Hibernus, Noctis Lilac and Noctis Lux. The existing Zed names remain stable.

## Reproduce and check

```sh
python3 scripts/generate_light_themes.py
python3 scripts/generate_light_themes.py --check
python3 scripts/test_light_themes.py
```
...
Only the three `*-light.json` variants are regenerated. Other variants retain
...
python3 scripts/inspect_rust_tokens.py \
  ...
  --output /tmp/noctis-semantic-preview.html
```

The implementation run used rust-analyzer `2026-01-19` and received **208 tokens**
from the fixture. All three palettes matched their reference semantic styles;
...
or copy the three light theme JSON files into Zed's `themes` configuration directory.
Select Hibernus Light, Lilac Light or Lux Light in the theme picker.
...
names, allowing all three themes to share exactly the same rules.
...
```sh
python3 scripts/preview.py --theme 'Lux Light'
```
...
- **Semantic styles:** resolved against the pinned VS Code and rust-analyzer
  mappings. Tests compare 16,128 token/modifier/theme combinations with the
...
- **UI:** `UI_MAP` identifies each direct source role. Zed-only controls and
  collaboration colors retain the existing port's choices. Layout, antialiasing
  and font rendering are editor-specific. Unsupported VS Code UI roles stay in
  the source snapshot for future work.
